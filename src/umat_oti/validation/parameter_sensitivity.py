...
def export_sensitivity_csv(
    ...
    Filenames carry a *method* suffix so downstream consumers cannot
    accidentally treat a finite-difference reference as an OTI-generated
    array. The mapping is:

    * ``run.backend == "centered_fd"`` -> ``DSIGMA_DP_FD.csv`` etc.
    * ``run.backend == "oti"``         -> ``DSIGMA_DP_OTI.csv`` etc.
    * ``run.backend == "analytical"``  -> ``DSIGMA_DP_ANALYTICAL.csv`` etc.

    The unsuffixed paper-facing ``DSIGMA_DP.csv`` / ``DSTATEV_DP.csv`` are
    generated by the higher-level evidence pipeline
    (:mod:`umat_oti.reports.run_softwarex_evidence`) *only after* an OTI run
    exists and its comparison against an independent reference passes.
    """
    ...
    suffix = {
        ...
        "hand_coded": "HAND",
    }.get(run.backend, run.backend.upper())

    dsigma_path = root / f"DSIGMA_DP_{suffix}.csv"
    dstatev_path = root / f"DSTATEV_DP_{suffix}.csv"
    primal_path = root / f"primal_stress_state_{suffix}.csv"

    with dsigma_path.open("w", newline="", encoding="utf-8") as fh:
        writer = csv.writer(fh)
        writer.writerow(
            ["increment", "stress_component", "method"] + list(param_names)
        )
        for inc in run.increments:
            for row, values in enumerate(inc.dsigma_dp, start=1):
                writer.writerow(
                    [inc.increment, row, run.backend] + [f"{v:.10e}" for v in values]
                )

    with dstatev_path.open("w", newline="", encoding="utf-8") as fh:
        writer = csv.writer(fh)
        writer.writerow(
            ["increment", "state_variable", "method"] + list(param_names)
        )
        for inc in run.increments:
            for row, name in enumerate(state_names):
                values = inc.dstatev_dp[row] if row < len(inc.dstatev_dp) else (0.0,) * len(param_names)
                writer.writerow(
                    [inc.increment, name, run.backend] + [f"{v:.10e}" for v in values]
                )

    with primal_path.open("w", newline="", encoding="utf-8") as fh:
        writer = csv.writer(fh)
        writer.writerow(
            ["increment", "yielded", "method"]
            + [f"stress_{i+1}" for i in range(NTENS)]
            + list(state_names)
        )
        for inc in run.increments:
            writer.writerow(
                [inc.increment, "yes" if inc.yielded else "no", run.backend]
                + [f"{v:.10e}" for v in inc.stress]
                + [f"{v:.10e}" for v in inc.statev[: len(state_names)]]
            )

    summary_path = root / f"sensitivity_summary_{suffix}.json"
    summary = {
        "backend": run.backend,
        "method": run.backend,
        ...
        "outputs": {
            f"DSIGMA_DP_{suffix}": dsigma_path.name,
            f"DSTATEV_DP_{suffix}": dstatev_path.name,
            f"primal_stress_state_{suffix}": primal_path.name,
        },
    }
    summary_path.write_text(json.dumps(summary, indent=2, sort_keys=True), encoding="utf-8")

    return {
        f"DSIGMA_DP_{suffix}": dsigma_path,
        f"DSTATEV_DP_{suffix}": dstatev_path,
        f"primal_{suffix}": primal_path,
        f"summary_{suffix}": summary_path,
    }
