...
| SoftwareX Table 6: parameter sensitivities across 18 models | 18 models; "76 of 84 directions" | **18/18 executed against an independently compiled original UMAT with primal parity passing; 17/18 derivatives verified; 72 of 76 parameter directions verified; 13 349 of 13 420 comparison rows agreeing** | `paper_results/parameter_sensitivity/{parameter_sensitivity_round.json,table6_parameter_sensitivity.csv,table6_comparison_rows.csv}` | `partially_implemented` | **Publish 72/76, not 76.** m6_fcc withholds 4 of its 10 directions: its centred-difference reference converges toward the OTI value as the step is refined (1.0e-5 to 2.1e-6) but plateaus, so those rows are reference-limited rather than wrong. The historical "84" belongs to the separate poster study |
