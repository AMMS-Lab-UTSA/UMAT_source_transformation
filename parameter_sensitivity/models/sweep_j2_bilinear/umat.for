C  ------------------------------------------------------------------------
C  sweep_j2_bilinear  Small-strain von Mises (J2) plasticity,
C  bilinear isotropic hardening via a tangent-modulus formulation.
C
C  PROPS(1)=E   PROPS(2)=nu   PROPS(3)=SIGY0 (initial yield)  PROPS(4)=Et
C  Plastic modulus derived from the tangent modulus:  H = E*Et/(E-Et).
C  STATEV(1) = EQPLAS (accumulated equivalent plastic strain)
C
C  Stress-driven radial return.  The incoming STRESS is the converged stress
C  of the previous increment; STRESS + Del*DSTRAN is the elastic predictor.
C  Closed-form return + consistent (algorithmic) tangent for linear (per-step)
C  hardening.  Voigt (11,22,33,12,13,23), engineering shear.  Self-contained.
C  ------------------------------------------------------------------------
...
C
      ...
      SIGY0=PROPS(3)
      ET=PROPS(4)
C     bilinear (tangent-modulus) plastic modulus
...
C       actively yielding: hydrostatic / deviatoric split + flow direction
        ...
        END DO
C       closed-form return for (per-step) linear hardening
