...
| Internal (local Newton) Jacobians of the ICP UMATs | OTI agrees with finite differences in all 21 (UMAT, Jacobian) pairs; 6 of the hand-coded Jacobians in those sources do not. Four drop a damage factor (1 − D); the other two, FJAC of UMAT_VPDCO and UMAT_VPDCL_R, are not the derivative of the residual their Newton loops evaluate | `python verification/run_all.py` (in Residual_Assembler) |
